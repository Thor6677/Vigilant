...
echo "[3/4] Recreate app with the restored image"
# --no-deps for the same reason deploy.sh uses it: only the app container
# cycles. TLS termination and routing live in a separate edge stack that this
# compose file does not own, so a Vigilant rollback must leave it alone.
docker compose up -d --no-deps --force-recreate app

echo "[4/4] Post-rollback checks"
sleep 3
if curl -fsS --max-time 5 http://127.0.0.1:8000/healthz >/dev/null 2>&1; then
    echo "     /healthz responded OK"
else
    echo "     WARNING: /healthz did not respond — check the logs below"
fi
