...
# Silence the host service-monitor for the duration of the deploy so the
...
# (or in the environment) to point at your ops toolkit's maintenance script.
# Unset, or not executable, and this step is skipped cleanly.
# shellcheck disable=SC1091
[ -r /opt/vigilant/.health-env ] && source /opt/vigilant/.health-env
MAINTENANCE="${MAINTENANCE:-}"
# Same source as health-check.sh, so an operator who overrides the container
# name in .health-env gets it honoured here too rather than having the
# post-deploy probes silently target a container that doesn't exist.
