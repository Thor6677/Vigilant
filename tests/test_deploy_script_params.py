...
def _default_for(src, var):
    """Extract `X` from a `VAR="${VAR:-X}"` assignment."""
    m = re.search(rf'^{var}="\$\{{{var}:-([^}}]*)\}}"', src, re.MULTILINE)
    ...
    for path in (DEPLOY, ROLLBACK):
        for i, line in enumerate(_src(path).splitlines(), 1):
            ...
            if stripped.startswith("#") or not stripped:
                continue
            if 'VIGILANT_ROOT="${VIGILANT_ROOT:-' in stripped:
                continue
...
def test_reexec_guard_still_precedes_any_git_use():
    ...
    for path, marker in ((DEPLOY, "VIGILANT_DEPLOY_REEXEC"), (ROLLBACK, "VIGILANT_ROLLBACK_REEXEC")):
        ...
        assert _first_executable_offset(src, marker) < first_git, \
            f"{path}: re-exec guard must appear before any git invocation"
