...
async def search_systems(request: Request, db: AsyncSession = Depends(get_db)):
    # T-047: gated for consistency with the rest of the module. This one only
    # reads local SDE, so there is no ESI budget to burn — but the parent page
    # is behind a session and a partial that outlives its page is how the
    # search/owners hole happened.
    if not request.session.get("user_id"):
        return HTMLResponse("", status_code=401)
    q = request.query_params.get("q", "").strip()
    if len(q) < 2:
...
async def search_regions(request: Request, db: AsyncSession = Depends(get_db)):
    # T-047: see search_systems — local SDE only, gated for consistency.
    if not request.session.get("user_id"):
        return HTMLResponse("", status_code=401)
    q = request.query_params.get("q", "").strip()
    if len(q) < 2:
...
async def search_owners(request: Request, db: AsyncSession = Depends(get_db)):
    # T-047 (primary): this endpoint borrows a stored character's token to run
    # an *authenticated* ESI search. Ungated, any anonymous visitor could loop
    # it and drain the instance-wide ESI error/rate budget, degrading every
    # sync on the deployment. The parent page already redirects without a
    # session; only this partial leaked. Gate before any ESI work happens.
    if not request.session.get("user_id"):
        return HTMLResponse("", status_code=401)
    q = request.query_params.get("q", "").strip()
    if len(q) < 3:
