...
async def character_detail(
    ...
    # helper opens its own AsyncSessionLocal for any DB work — sharing one
    # AsyncSession across concurrent coroutines that write is unsafe. Helpers
    # that need ESI caching set
    ...
    async def _fetch_implants_clones() -> tuple[list, list]:
        ...
        try:
            ...
            # Parallel structure-name lookups. Bounded concurrency deliberately:
            # unbounded fan-out to one ESI endpoint group earns 429s.
            if structure_ids:
