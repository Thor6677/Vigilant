...
# UA is built at request time via user_agent("backfill") — the "backfill" token
# lets the remote operator tell bulk historical fetches from interactive traffic,
# and the contact address comes from the CONTACT_EMAIL setting.
...
async def _fetch_history(rng: str, include_newborns: bool = True) -> list:
    ...
    async with httpx.AsyncClient(
        timeout=60.0,
        headers={"User-Agent": user_agent("backfill"), "Accept": "application/json"},
    ) as client:
