...
def _zkb_headers() -> dict[str, str]:
    """zKillboard request headers, built per call.

    A function rather than a module-level dict so the contactable User-Agent is
    resolved at request time from the CONTACT_EMAIL setting — building it at
    import time would couple module import order to config availability.
    """
    return {
...
async def zkb_get(path: str) -> list:
    ...
    async with _zkb_sem:
        ...
        try:
            resp = await client.get(url, headers=_zkb_headers(), timeout=15)
