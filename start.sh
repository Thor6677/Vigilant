...
# .env.example ships the PRODUCTION callback placeholder, but this script runs
# the app locally. Without this, a clean `git clone && ./start.sh` leaves
# EVE_CALLBACK_URL pointing at https://yourdomain.com/auth/callback and the very
# first SSO login fails — EVE requires the callback to match the one registered
# on the developer application exactly.
callback=$(_env_get EVE_CALLBACK_URL)
if [ -z "$callback" ] || [ "$callback" = "https://yourdomain.com/auth/callback" ]; then
    _env_set EVE_CALLBACK_URL "http://localhost:8000/auth/callback"
    echo "Set EVE_CALLBACK_URL to http://localhost:8000/auth/callback for local dev."
fi
